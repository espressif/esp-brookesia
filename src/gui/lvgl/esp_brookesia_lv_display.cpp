...
#if !ESP_BROOKESIA_CONF_LVGL_DISPLAY_ENABLE_DEBUG_LOG
...
{
    ESP_BROOKESIA_CHECK_NULL_RETURN(touch, false, "Invalid touch device");
...
process_height:
    ...
    if (target.flags.enable_height_percent) {
        ESP_BROOKESIA_CHECK_NULL_RETURN(parent, false, "Invalid parent size");
...
{
    ESP_BROOKESIA_LOGD("Update lvgl display by new data");
...
const lv_font_t *LvDisplay::getFontBySize(uint8_t size_px) const
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_RETURN(it != _size_font_map.end(), nullptr, "Font size(%d) is not found", size_px);
...
const lv_font_t *LvDisplay::getFontByHeight(uint8_t height, uint8_t *size_px) const
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_RETURN(lower != _height_font_map.end(), nullptr, "Font height(%d) is not found", height);
