...
#if !ESP_BROOKESIA_CONF_LVGL_TIMER_ENABLE_DEBUG_LOG
    ...
    _user_data{this, user_data}
{
    ...
    ESP_BROOKESIA_CHECK_NULL_EXIT(callback, "Invalid callback");
    _native_handle = lv_timer_create([](lv_timer_t *t) {
        ESP_BROOKESIA_LOG_TRACE_ENTER();

        LvTimer *timer = (LvTimer *)t->user_data;
        ESP_BROOKESIA_CHECK_NULL_EXIT(timer, "Invalid timer");

        timer->_callback(timer->_user_data.user_data);

        ESP_BROOKESIA_LOG_TRACE_EXIT();
    }, period, this);
...
bool LvTimer::pause()
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_RETURN(isValid(), false, "Invalid timer");
