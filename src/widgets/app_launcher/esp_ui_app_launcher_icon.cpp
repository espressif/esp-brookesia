...
ESP_UI_AppLauncherIcon::ESP_UI_AppLauncherIcon(ESP_UI_Core &core, const ESP_UI_AppLauncherIconInfo_t &info,
        const ESP_UI_AppLauncherIconData_t &data):
    _core(core),
    _info(info),
    _data(data),
...
ESP_UI_AppLauncherIcon::~ESP_UI_AppLauncherIcon()
{
...
bool ESP_UI_AppLauncherIcon::begin(lv_obj_t *parent)
{
    ESP_UI_LvObj_t main_obj = nullptr;
    ESP_UI_LvObj_t icon_main_obj = nullptr;
    ESP_UI_LvObj_t icon_image_obj = nullptr;
    ESP_UI_LvObj_t name_label = nullptr;

    ESP_UI_LOGD("Begin(%d: @0x%p)", _info.id, this);
    ESP_UI_CHECK_NULL_RETURN(parent, false, "Invalid parent object");
    ESP_UI_CHECK_NULL_RETURN(_info.name, false, "Invalid name");
    ESP_UI_CHECK_NULL_RETURN(_info.image.resource, false, "Invalid image resource");
    ...
    // Icon
    icon_main_obj = ESP_UI_LV_OBJ(obj, main_obj.get());
    ESP_UI_CHECK_NULL_RETURN(icon_main_obj.get(), false, "Create icon_main_obj failed");
    // Image
    icon_image_obj = ESP_UI_LV_OBJ(img, icon_main_obj.get());
    ESP_UI_CHECK_NULL_RETURN(icon_image_obj, false, "Create icon_image_obj failed");
    // Name
    name_label = ESP_UI_LV_OBJ(label, main_obj.get());
    ESP_UI_CHECK_NULL_RETURN(name_label, false, "Create name_label failed");
...
err:
    ESP_UI_CHECK_FALSE_RETURN(del(), false, "Delete icon failed");

    return false;
}

bool ESP_UI_AppLauncherIcon::del(void)
{
    ESP_UI_LOGD("Delete(%d: @0x%p)", _info.id, this);
...
bool ESP_UI_AppLauncherIcon::updateByNewData(void)
{
    float h_factor = 0;
    float w_factor = 0;

    ESP_UI_LOGD("Update(%d: @0x%p)", _info.id, this);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Icon is not initialized");
...
void ESP_UI_AppLauncherIcon::onIconTouchEventCallback(lv_event_t *event)
{
    ESP_UI_AppLauncherIcon *icon = nullptr;
    lv_event_code_t event_code = _LV_EVENT_LAST;
    lv_obj_t *icon_image_obj = nullptr;
    ESP_UI_CoreAppEventData_t app_event_data = {
        .type = ESP_UI_CORE_APP_EVENT_TYPE_START,
    };

    ESP_UI_LOGD("Icon touch event callback");
    ESP_UI_CHECK_NULL_EXIT(event, "Invalid event object");

    icon = (ESP_UI_AppLauncherIcon *)lv_event_get_user_data(event);
    event_code = lv_event_get_code(event);
    icon_image_obj = (lv_obj_t *)lv_event_get_current_target(event);
    ESP_UI_CHECK_NULL_EXIT(icon, "Invalid icon");
    ESP_UI_CHECK_NULL_EXIT(icon_image_obj, "Invalid icon image");
    ...
    case LV_EVENT_CLICKED:
        ...
        if (!icon->_is_pressed_losted) {
            ...
            ESP_UI_CHECK_FALSE_EXIT(icon->_core.sendAppEvent(&app_event_data), "Send app event failed");
        }
        break;
