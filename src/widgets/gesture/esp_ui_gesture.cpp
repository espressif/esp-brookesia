...
ESP_UI_Gesture::ESP_UI_Gesture(ESP_UI_Core &core_in, const ESP_UI_GestureData_t &data_in):
    core(core_in),
    data(data_in),
...
bool ESP_UI_Gesture::begin(lv_obj_t *parent)
{
    ...
    array<ESP_UI_LvObj_t, ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX> indicator_bars = {};
    array<ESP_UI_LvAnim_t, ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX> indicator_bar_scale_back_anims = {};
    ...
    ESP_UI_CHECK_NULL_RETURN(core.getTouchDevice(), false, "Invalid core touch device");

    /* Create objects */
    detect_timer = ESP_UI_LV_TIMER(onTouchDetectTimerCallback, data.detect_period_ms, this);
    ...
    for (int i = 0; i < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX; i++) {
        indicator_bars[i] = ESP_UI_LV_OBJ(bar, parent);
        ESP_UI_CHECK_NULL_RETURN(indicator_bars[i], false, "Create indicator bar failed");
        indicator_bar_scale_back_anims[i] = ESP_UI_LV_ANIM();
        ESP_UI_CHECK_NULL_RETURN(indicator_bar_scale_back_anims[i], false, "Create indicator bar animation failed");
        _indicator_bar_anim_var[i] = {
            .gesture = this,
            .type = (ESP_UI_GestureIndicatorBarType_t)i,
        };
    }
...
bool ESP_UI_Gesture::checkMaskVisible(void) const
{
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
...
bool ESP_UI_Gesture::checkIndicatorBarVisible(ESP_UI_GestureIndicatorBarType_t type) const
{
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(type < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX, false, "Invalid indicator bar type");

    return !lv_obj_has_flag(_indicator_bars[type].get(), LV_OBJ_FLAG_HIDDEN);
}

int ESP_UI_Gesture::getIndicatorBarLength(ESP_UI_GestureIndicatorBarType_t type) const
{
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), -1, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(type < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX, -1, "Invalid indicator bar type");
    ...
    } else if (type == ESP_UI_GESTURE_INDICATOR_BAR_TYPE_BOTTOM) {
...
bool ESP_UI_Gesture::calibrateData(const ESP_UI_StyleSize_t &screen_size, const ESP_UI_CoreHome &home,
                                   ESP_UI_GestureData_t &data)
{
    ...
    for (int i = 0; i < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX; i++) {
        ...
        ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(screen_size, data.indicator_bars[i].main.size_max), false,
                                  "Calibrate indicator bar main size max failed");
        ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(screen_size, data.indicator_bars[i].main.size_min, true),
                                  false, "Calibrate indicator bar main size min failed");
        parent_size = &data.indicator_bars[i].main.size_min;
        parent_w = parent_size->width;
        parent_h = parent_size->height;
        if (i == ESP_UI_GESTURE_INDICATOR_BAR_TYPE_LEFT || i == ESP_UI_GESTURE_INDICATOR_BAR_TYPE_RIGHT) {
            ESP_UI_CHECK_VALUE_RETURN(data.indicator_bars[i].main.align_offset, 0, parent_h / 2, false,
                                      "Invalid indicator bar main align offset");
        } else if (i == ESP_UI_GESTURE_INDICATOR_BAR_TYPE_BOTTOM) {
            ESP_UI_CHECK_VALUE_RETURN(data.indicator_bars[i].main.align_offset, 0, parent_w / 2, false,
                                      "Invalid indicator bar main align offset");
        }
        ESP_UI_CHECK_VALUE_RETURN(data.indicator_bars[i].main.layout_pad_all, 0, parent_h / 2, false,
                                  "Invalid indicator bar main layout pad all");
        ESP_UI_CHECK_VALUE_RETURN(data.indicator_bars[i].main.layout_pad_all, 0, parent_w / 2, false,
                                  "Invalid indicator bar main layout pad all");
    }
...
bool ESP_UI_Gesture::setIndicatorBarLength(ESP_UI_GestureIndicatorBarType_t type, uint16_t length) const
{
    ESP_UI_LOGD("Set indicator bar(%d) length(%d)", type, length);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(type < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX, -1, "Invalid indicator bar type");
...
bool ESP_UI_Gesture::setIndicatorBarLengthByOffset(ESP_UI_GestureIndicatorBarType_t type, int offset) const
{
    ESP_UI_LOGD("Set indicator bar(%d) length by offset(%d)", type, offset);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(type < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX, -1, "Invalid indicator bar type");
    ...
    switch (type) {
    case ESP_UI_GESTURE_INDICATOR_BAR_TYPE_LEFT:
    case ESP_UI_GESTURE_INDICATOR_BAR_TYPE_RIGHT:
        offset = max(0, min(offset, (int)data.threshold.direction_horizon));
        max_len = data.indicator_bars[type].main.size_max.height;
        break;
    case ESP_UI_GESTURE_INDICATOR_BAR_TYPE_BOTTOM:
...
bool ESP_UI_Gesture::setIndicatorBarVisible(ESP_UI_GestureIndicatorBarType_t type, bool visible)
{
    ESP_UI_LOGD("Set indicator bar(%d) visible(%d)", type, visible);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ...
    if (visible) {
        ...
        ESP_UI_CHECK_FALSE_RETURN(setIndicatorBarLength(type, _indicator_bar_max_lengths[type]), false,
...
bool ESP_UI_Gesture::controlIndicatorBarScaleBackAnim(ESP_UI_GestureIndicatorBarType_t type, bool start)
{
    int length = 0;

    ESP_UI_LOGD("Control indicator bar(%d) scale back animation(%d)", type, start);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ...
    ESP_UI_CHECK_FALSE_RETURN(length >= 0, false, "Get indicator bar length failed");
    ...
    if (start) {
        ...
        if (length == _indicator_bar_max_lengths[type]) {
            ESP_UI_CHECK_FALSE_RETURN(setIndicatorBarVisible(type, false), false, "Set indicator bar visible failed");

            return true;
        }
        lv_anim_set_values(_indicator_bar_scale_back_anims[type].get(), length, _indicator_bar_max_lengths[type]);
        ESP_UI_CHECK_NULL_RETURN(lv_anim_start(_indicator_bar_scale_back_anims[type].get()), false,
...
bool ESP_UI_Gesture::updateByNewData(void)
{
    ...
    // Indicator bar
    for (int i = 0; i < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX; i++) {
        const ESP_UI_GestureIndicatorBarData_t &bar_data = data.indicator_bars[i];
        // Main
        ...
        lv_anim_set_path_cb(_indicator_bar_scale_back_anims[i].get(),
                            esp_ui_core_utils_get_anim_path_cb(bar_data.animation.scale_back_path_type));
        lv_anim_set_time(_indicator_bar_scale_back_anims[i].get(), bar_data.animation.scale_back_time_ms);
        // Others
        if (i == ESP_UI_GESTURE_INDICATOR_BAR_TYPE_LEFT) {
        ...
        } else if (i == ESP_UI_GESTURE_INDICATOR_BAR_TYPE_RIGHT) {
        ...
        ESP_UI_CHECK_FALSE_RETURN(bar_range > 0, false, "Invalid bar range");
...
void ESP_UI_Gesture::onTouchDetectTimerCallback(struct _lv_timer_t *t)
{
    ...
    const ESP_UI_GestureData_t &data = gesture->data;
...
void ESP_UI_Gesture::onIndicatorBarScaleBackAnimationExecuteCallback(void *var, int32_t value)
{
    auto anim_var = static_cast<IndicatorBarAnimVar_t *>(var);
    ESP_UI_Gesture *gesture = nullptr;
    ESP_UI_GestureIndicatorBarType_t type = ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX;

    ESP_UI_CHECK_NULL_EXIT(anim_var, "Invalid var");

    gesture = anim_var->gesture;
    ESP_UI_CHECK_NULL_EXIT(gesture, "Invalid gesture");
    type = anim_var->type;
    ESP_UI_CHECK_FALSE_EXIT(type < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX, "Invalid indicator bar type");
...
void ESP_UI_Gesture::onIndicatorBarScaleBackAnimationReadyCallback(lv_anim_t *anim)
{
    ESP_UI_Gesture *gesture = nullptr;
    ESP_UI_GestureIndicatorBarType_t type = ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX;

    ESP_UI_LOGD("Indicator bar scale back animation ready callback");
    ESP_UI_CHECK_NULL_EXIT(anim, "Invalid anim");

    auto anim_var = static_cast<IndicatorBarAnimVar_t *>(anim->var);
    ESP_UI_CHECK_NULL_EXIT(anim_var, "Invalid user data");
    gesture = anim_var->gesture;
    ESP_UI_CHECK_NULL_EXIT(gesture, "Invalid gesture");
    type = anim_var->type;
    ESP_UI_CHECK_FALSE_EXIT(type < ESP_UI_GESTURE_INDICATOR_BAR_TYPE_MAX, "Invalid indicator bar type");
    ...
    ESP_UI_CHECK_FALSE_EXIT(gesture->setIndicatorBarVisible(type, false), "Hide indicator bar failed");
}
