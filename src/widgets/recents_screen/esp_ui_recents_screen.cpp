...
#include "esp_ui_recents_screen.hpp"
...
ESP_UI_RecentsScreen::ESP_UI_RecentsScreen(ESP_UI_Core &core, const ESP_UI_RecentsScreenData_t &data):
    _core(core),
    _data(data),
...
ESP_UI_RecentsScreen::~ESP_UI_RecentsScreen()
{
...
bool ESP_UI_RecentsScreen::begin(lv_obj_t *parent)
{
    ESP_UI_LvObj_t main_obj = nullptr;
    ESP_UI_LvObj_t memory_obj = nullptr;
    ESP_UI_LvObj_t memory_label = nullptr;
    ESP_UI_LvObj_t snapshot_table = nullptr;
    ESP_UI_LvObj_t trash_obj = nullptr;
    ESP_UI_LvObj_t trash_icon = nullptr;

    ESP_UI_LOGD("Begin(0x%p)", this);
    ESP_UI_CHECK_NULL_RETURN(parent, false, "Invalid parent object");
    ESP_UI_CHECK_FALSE_RETURN(!checkInitialized(), false, "RecentsScreen is already initialized");
    ...
    if (_data.flags.enable_memory) {
        ESP_UI_LOGD("Enable memory label");
        memory_obj = ESP_UI_LV_OBJ(obj, main_obj.get());
        ESP_UI_CHECK_NULL_RETURN(main_obj, false, "Create main object failed");
        memory_label = ESP_UI_LV_OBJ(label, memory_obj.get());
        ESP_UI_CHECK_NULL_RETURN(memory_label, false, "Create memory label failed");
    }
    // Snapshot snapshot_table
    snapshot_table = ESP_UI_LV_OBJ(obj, main_obj.get());
    ESP_UI_CHECK_NULL_RETURN(snapshot_table, false, "Create snapshot snapshot_table failed");
    // Trash
    trash_obj = ESP_UI_LV_OBJ(obj, main_obj.get());
    ESP_UI_CHECK_NULL_RETURN(trash_obj, false, "Create trash_icon object failed");
    trash_icon = ESP_UI_LV_OBJ(img, trash_obj.get());
    ESP_UI_CHECK_NULL_RETURN(trash_icon, false, "Create trash_icon icon failed");
...
bool ESP_UI_RecentsScreen::del(void)
{
    bool ret = true;
...
bool ESP_UI_RecentsScreen::setVisible(bool visible) const
{
...
bool ESP_UI_RecentsScreen::addSnapshot(const ESP_UI_RecentsScreenSnapshotConf_t &conf)
{
    shared_ptr<ESP_UI_RecentsScreenSnapshot> snapshot = nullptr;

    ESP_UI_LOGD("Add snapshot(%d)", conf.id);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");

    snapshot = make_shared<ESP_UI_RecentsScreenSnapshot>(_core, conf, _data.snapshot_table.snapshot);
    ESP_UI_CHECK_NULL_RETURN(snapshot, false, "Create snapshot failed");

    ESP_UI_CHECK_FALSE_RETURN(snapshot->begin(_snapshot_table.get()), false, "Begin snapshot failed");

    if (checkSnapshotExist(conf.id)) {
        ESP_UI_LOGW("Already exist, override it");
        _id_snapshot_map[conf.id] = snapshot;
    } else {
        auto ret = _id_snapshot_map.insert(std::pair<int, shared_ptr<ESP_UI_RecentsScreenSnapshot>>(conf.id, snapshot));
        ESP_UI_CHECK_FALSE_RETURN(ret.second, false, "Insert snapshot failed");
    }

    ESP_UI_CHECK_FALSE_RETURN(scrollToSnapshotById(conf.id), false, "Scroll to snapshot failed");

    return true;
}

bool ESP_UI_RecentsScreen::removeSnapshot(int id)
{
    ESP_UI_LOGD("Remove snapshot(%d)", id);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(checkSnapshotExist(id), false, "Snapshot is not exist");

    int num = _id_snapshot_map.erase(id);
    ESP_UI_CHECK_FALSE_RETURN(num > 0, false, "Remove snapshot failed");

    return true;
}

bool ESP_UI_RecentsScreen::scrollToSnapshotById(int id)
{
    lv_obj_t *snapshot_main_obj = nullptr;

    ESP_UI_LOGD("Scroll to snapshot id(%d)", id);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(checkSnapshotExist(id), false, "Snapshot is not exist");

    snapshot_main_obj = _id_snapshot_map.at(id)->getMainObj();
    ESP_UI_CHECK_NULL_RETURN(snapshot_main_obj, false, "Invalid snapshot main object");
...
bool ESP_UI_RecentsScreen::scrollToSnapshotByIndex(uint8_t index)
{
    lv_obj_t *snapshot_main_obj = nullptr;

    ESP_UI_LOGD("Scroll to snapshot index(%d)", index);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(index < _id_snapshot_map.size(), false, "Invalid snapshot index");
...
bool ESP_UI_RecentsScreen::moveSnapshotY(int id, int y)
{
    lv_obj_t *drag_obj = NULL;

    ESP_UI_LOGD("Move snapshot(%d) to y(%d)", id, y);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(checkSnapshotExist(id), false, "Snapshot is not exist");

    drag_obj = _id_snapshot_map.at(id)->getDragObj();
    ESP_UI_CHECK_NULL_RETURN(drag_obj, false, "Invalid snapshot drag object");
...
bool ESP_UI_RecentsScreen::updateSnapshotImage(int id)
{
    ESP_UI_LOGD("Update snapshot(%d) image", id);
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(checkSnapshotExist(id), false, "Snapshot is not exist");

    ESP_UI_CHECK_FALSE_RETURN(_id_snapshot_map.at(id)->updateByNewData(), false, "Update snapshot style failed");

    return true;
}

bool ESP_UI_RecentsScreen::setMemoryLabel(int internal_free, int internal_total, int external_free, int external_total) const
{
    ESP_UI_LOGD("Set memory label");
    ESP_UI_CHECK_FALSE_RETURN(_memory_label != nullptr, false, "Memory label is disabled");
...
bool ESP_UI_RecentsScreen::checkSnapshotExist(int id) const
{
...
bool ESP_UI_RecentsScreen::checkVisible(void) const
{
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), false, "Not initialized");
...
bool ESP_UI_RecentsScreen::checkPointInsideMain(lv_point_t &point) const
{
...
bool ESP_UI_RecentsScreen::checkPointInsideTable(lv_point_t &point) const
{
    lv_area_t area = {};
...
bool ESP_UI_RecentsScreen::checkPointInsideSnapshot(int id, lv_point_t &point) const
{
    lv_area_t area = {};
    ...
    snapshot_main_obj = _id_snapshot_map.at(id)->getMainObj();
    ESP_UI_CHECK_FALSE_RETURN(snapshot_main_obj != NULL, false, "Invalid snapshot main object");
...
int ESP_UI_RecentsScreen::getSnapshotOriginY(int id) const
{
    ESP_UI_CHECK_FALSE_RETURN(checkSnapshotExist(id), 0, "Snapshot is not exist");

    return _id_snapshot_map.at(id)->getOriginY();
}

int ESP_UI_RecentsScreen::getSnapshotCurrentY(int id) const
{
    ESP_UI_CHECK_FALSE_RETURN(checkSnapshotExist(id), 0, "Snapshot is not exist");
...
int ESP_UI_RecentsScreen::getSnapshotIdPointIn(lv_point_t &point) const
{
    ESP_UI_CHECK_FALSE_RETURN(checkInitialized(), -1, "Not initialized");
...
bool ESP_UI_RecentsScreen::calibrateData(const ESP_UI_StyleSize_t &screen_size, const ESP_UI_CoreHome &home,
        ESP_UI_RecentsScreenData_t &data)
{
    uint16_t parent_w = 0;
    uint16_t parent_h = 0;
    ESP_UI_RecentsScreenSnapshotData_t &new_snapshot_data = data.snapshot_table.snapshot;
    ...
    parent_h = parent_size->height;
    // Layout
    ESP_UI_CHECK_VALUE_RETURN(data.main.layout_row_pad, 0, parent_h, false, "Invalid main layout row pad");
    ESP_UI_CHECK_VALUE_RETURN(data.main.layout_top_pad, 0, parent_h, false, "Invalid main layout top pad");
    ESP_UI_CHECK_VALUE_RETURN(data.main.layout_bottom_pad, 0, parent_h, false, "Invalid main layout bottom pad");
    ...
    if (data.flags.enable_memory) {
        ...
        parent_h = parent_size->height;
        // Main
        ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, data.memory.main_size), false,
                                  "Invalid memory main size");
        parent_size = &data.memory.main_size;
        parent_w = parent_size->width;
        parent_h = parent_size->height;
        ESP_UI_CHECK_VALUE_RETURN(data.memory.main_layout_x_right_offset, 0, parent_w, false,
                                  "Invalid memory main layout x right offset");
        // Label
        ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreFont(parent_size, data.memory.label_text_font), false,
                                  "Invalid memory label text font size");
        ESP_UI_CHECK_NULL_RETURN(data.memory.label_unit_text, false, "Invalid memory label unit text");
    }
    ...
    parent_w = parent_size->width;
    parent_h = parent_size->height;
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, data.trash_icon.default_size), false,
                              "Invalid trash icon default size");
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, data.trash_icon.press_size), false,
                              "Invalid trash icon eprbdefault size");
    ESP_UI_CHECK_NULL_RETURN(data.trash_icon.image.resource, false, "Invalid trash icon image resource");
    ...
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, data.snapshot_table.main_size), false,
                              "Invalid snapshot_table main size");
    parent_size = &data.snapshot_table.main_size;
    parent_w = parent_size->width;
    ESP_UI_CHECK_VALUE_RETURN(data.snapshot_table.main_layout_column_pad, 0, parent_w, false,
    ...
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, new_snapshot_data.main_size), false,
    ...
    if (new_snapshot_data.flags.enable_all_main_size_refer_screen) {
        ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.main_size.width, 1, data.snapshot_table.main_size.width, false,
                                  "Invalid snapshot main width");
        ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.main_size.height, 1, data.snapshot_table.main_size.height, false,
                                  "Invalid snapshot main height");
    }
    // Title
    parent_size = (new_snapshot_data.flags.enable_all_main_size_refer_screen) ? &screen_size :
                  &new_snapshot_data.main_size;
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, new_snapshot_data.title.main_size), false,
                              "Invalid snapshot title size");
    // If referring to the screen, check with the main size
    if (new_snapshot_data.flags.enable_all_main_size_refer_screen) {
        ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.title.main_size.width, 1, new_snapshot_data.main_size.width, false,
                                  "Invalid snapshot title main width");
        ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.title.main_size.height, 1, new_snapshot_data.main_size.height, false,
                                  "Invalid snapshot title main height");
    }
    parent_size = &new_snapshot_data.title.main_size;
    parent_w = parent_size->width;
    ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.title.main_layout_column_pad, 0, parent_w, false,
                              "Invalid snapshot title layout column pad");
    // Title Icon
    parent_size = &new_snapshot_data.title.main_size;
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, new_snapshot_data.title.icon_size), false,
                              "Invalid snapshot title icon size");
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreFont(parent_size, new_snapshot_data.title.text_font), false,
                              "Invalid snapshot title text font");
    // Image
    parent_size = (new_snapshot_data.flags.enable_all_main_size_refer_screen) ? &screen_size :
                  &new_snapshot_data.main_size;
    ESP_UI_CHECK_FALSE_RETURN(home.calibrateCoreObjectSize(*parent_size, new_snapshot_data.image.main_size), false,
                              "Invalid snapshot image main size");
    // If referring to the screen, check with the main size
    if (new_snapshot_data.flags.enable_all_main_size_refer_screen) {
        ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.image.main_size.width, 1, new_snapshot_data.main_size.width, false,
                                  "Invalid snapshot image main width");
        ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.image.main_size.height, 1, new_snapshot_data.main_size.height, false,
                                  "Invalid snapshot image main height");
    }
    // All
    parent_size = &new_snapshot_data.main_size;
    parent_h = parent_size->height;
    ESP_UI_CHECK_VALUE_RETURN(new_snapshot_data.title.main_size.height + new_snapshot_data.image.main_size.height, 1,
...
bool ESP_UI_RecentsScreen::updateByNewData(void)
{
    float h_factor = 0;
    float w_factor = 0;
    ...
    for (auto &it : _id_snapshot_map) {
        ESP_UI_CHECK_NULL_RETURN(it.second, false, "Invalid snapshot(%d)", it.first);
        ESP_UI_CHECK_FALSE_RETURN(it.second->updateByNewData(), false, "Update snapshot object style failed");
    }

    return true;
}

void ESP_UI_RecentsScreen::onDataUpdateEventCallback(lv_event_t *event)
{
    ESP_UI_RecentsScreen *recents_screen = nullptr;

    ESP_UI_LOGD("Data update event");
    ESP_UI_CHECK_NULL_EXIT(event, "Invalid event object");

    recents_screen = (ESP_UI_RecentsScreen *)lv_event_get_user_data(event);
    ESP_UI_CHECK_NULL_EXIT(recents_screen, "Invalid app snapshot_table object");

    ESP_UI_CHECK_FALSE_EXIT(recents_screen->updateByNewData(), "Update object style failed");
}

void ESP_UI_RecentsScreen::onTrashTouchEventCallback(lv_event_t *event)
{
    lv_event_code_t event_code = lv_event_get_code(event);
    ESP_UI_RecentsScreen *recents_screen = (ESP_UI_RecentsScreen *)lv_event_get_user_data(event);
    std::unordered_map<int, std::shared_ptr<ESP_UI_RecentsScreenSnapshot>> id_snapshot_map;

    ESP_UI_LOGD("Trash touch event callback");
    ESP_UI_CHECK_NULL_EXIT(recents_screen, "Invalid recents_screen object");
