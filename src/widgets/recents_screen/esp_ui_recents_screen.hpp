...
// *INDENT-OFF*
class ESP_UI_RecentsScreen {
public:
    ESP_UI_RecentsScreen(ESP_UI_Core &core, const ESP_UI_RecentsScreenData_t &data);
    ~ESP_UI_RecentsScreen();

    bool begin(lv_obj_t *parent);
    bool del(void);

    bool setVisible(bool visible) const;
    bool addSnapshot(const ESP_UI_RecentsScreenSnapshotConf_t &conf);
...
private:
    ...
    ESP_UI_Core &_core;
    const ESP_UI_RecentsScreenData_t &_data;
    ...
    ESP_UI_LvObj_t _memory_obj;
    ESP_UI_LvObj_t _memory_label;
    ESP_UI_LvObj_t _snapshot_table;
    ESP_UI_LvObj_t _trash_obj;
    ESP_UI_LvObj_t _trash_icon;
    std::unordered_map<int, std::shared_ptr<ESP_UI_RecentsScreenSnapshot>> _id_snapshot_map;
};
// *INDENT-OFF*
