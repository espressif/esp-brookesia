...
ESP_UI_StatusBarIcon::ESP_UI_StatusBarIcon(const ESP_UI_StatusBarIconData_t &data):
    _data(data),
...
ESP_UI_StatusBarIcon::~ESP_UI_StatusBarIcon()
{
...
bool ESP_UI_StatusBarIcon::begin(const ESP_UI_Core &core, lv_obj_t *parent)
{
    ESP_UI_LvObj_t main_obj = nullptr;
    ESP_UI_LvObj_t image_obj = nullptr;
    vector<ESP_UI_LvObj_t> image_objs;

    ESP_UI_LOGD("Begin(@0x%p)", this);
    ESP_UI_CHECK_NULL_RETURN(parent, false, "Invalid parent");
    ESP_UI_CHECK_FALSE_RETURN(!checkInitialized(), false, "Icon is already initialized");
    ...
    for (int i = 0; i < _data.icon.image_num; i++) {
        image_obj = ESP_UI_LV_OBJ(img, main_obj.get());
        ESP_UI_CHECK_NULL_RETURN(image_obj, false, "Create icon image[%d] failed", i);
...
bool ESP_UI_StatusBarIcon::del(void)
{
...
bool ESP_UI_StatusBarIcon::setCurrentState(int state)
{
    uint8_t image_resource_num = _image_objs.size();
    lv_obj_t *img_obj = nullptr;

    ESP_UI_LOGD("Set state(%d)", state);
    ESP_UI_CHECK_FALSE_RETURN(state < image_resource_num, false, "Invalid state(%d)", state);
    ESP_UI_CHECK_NULL_RETURN(_main_obj, false, "Invalid main object");
...
bool ESP_UI_StatusBarIcon::updateByNewData(void)
{
    int image_resource_num = _image_objs.size();
    float h_factor = 0;
    float w_factor = 0;
    const lv_img_dsc_t *img_dsc = nullptr;
    ESP_UI_LvObj_t main_obj = _main_obj;
    ESP_UI_LvObj_t image_obj = nullptr;
    ...
    if (esp_ui_core_utils_check_obj_out_of_parent(main_obj.get())) {
        _is_out_of_parent = true;
        lv_obj_add_flag(main_obj.get(), LV_OBJ_FLAG_HIDDEN);
        ESP_UI_LOGW("Icon out of area, hide it");
    }
