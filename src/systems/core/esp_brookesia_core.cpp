...
#if !ESP_BROOKESIA_CONF_CORE_CORE_ENABLE_DEBUG_LOG
...
bool ESP_Brookesia_Core::getDisplaySize(ESP_Brookesia_StyleSize_t &size)
{
    ...
    if (_display_device == nullptr) {
        ...
        ESP_BROOKESIA_CHECK_NULL_RETURN(_display_device, false, "Display device is not initialized");
    }
...
bool ESP_Brookesia_Core::beginCore(void)
{
    ...
    // Initialize cores
    ESP_BROOKESIA_CHECK_FALSE_GOTO(_core_display.beginCore(), err, "Begin core home failed");
    ESP_BROOKESIA_CHECK_FALSE_GOTO(_core_manager.beginCore(), err, "Begin core manager failed");

    // Initialize others
#if ESP_BROOKESIA_CONF_GUI_SQUARELINE_ENABLE_UI_COMP
...
bool ESP_Brookesia_Core::calibrateCoreData(ESP_Brookesia_CoreData_t &data)
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_RETURN(_core_display.calibrateCoreObjectSize(display_size, data.screen_size), false,
                                     "Invalid Core screen_size");

    // Home
    ESP_BROOKESIA_CHECK_FALSE_RETURN(_core_display.calibrateCoreData(data.home), false, "Invalid Core home data");
...
void ESP_Brookesia_Core::onCoreDataUpdateEventCallback(lv_event_t *event)
{
    ...
    ESP_BROOKESIA_CHECK_FALSE_EXIT(core->_core_display.updateByNewData(), "Core home update failed");
}
