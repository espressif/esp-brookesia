...
#if !ESP_UI_LOG_ENABLE_DEBUG_CORE_HOME
#undef ESP_UI_LOGD
#define ESP_UI_LOGD(...)
#endif
...
ESP_UI_CoreHome::ESP_UI_CoreHome(ESP_UI_Core &core, const ESP_UI_CoreHomeData_t &data):
    _core(core),
    _core_data(data),
...
ESP_UI_CoreHome::~ESP_UI_CoreHome()
{
...
bool ESP_UI_CoreHome::showContainerBorder(void)
{
    ESP_UI_LOGD("Show container border");
    ESP_UI_CHECK_FALSE_RETURN(checkCoreInitialized(), false, "Not initialized");
...
bool ESP_UI_CoreHome::hideContainerBorder(void)
{
    ESP_UI_LOGD("Hide container border");
    ESP_UI_CHECK_FALSE_RETURN(checkCoreInitialized(), false, "Not initialized");
...
lv_style_t *ESP_UI_CoreHome::getCoreContainerStyle(void)
{
...
const lv_font_t *ESP_UI_CoreHome::getCoreDefaultFontBySize(uint8_t size) const
{
    ESP_UI_CHECK_VALUE_RETURN(size, ESP_UI_STYLE_FONT_SIZE_MIN, ESP_UI_STYLE_FONT_SIZE_MAX, nullptr, "Invalid size");

    auto it = _default_size_font_map.find(size);
    ESP_UI_CHECK_FALSE_RETURN(it != _default_size_font_map.end(), nullptr, "Font size(%d) is not found", size);

    return it->second;
}

const lv_font_t *ESP_UI_CoreHome::getCoreDefaultFontByHeight(uint8_t height, uint8_t *size_px) const
{
    uint8_t ret_size = 0;

    ESP_UI_CHECK_NULL_RETURN(size_px, nullptr, "Invalid size_px");

    auto lower = _default_height_font_map.lower_bound(height);
    if ((lower->first != height) && (lower != _default_height_font_map.begin())) {
        lower--;
    }
    ESP_UI_CHECK_FALSE_RETURN(lower != _update_height_font_map.end(), nullptr, "Font height(%d) is not found", height);
    ...
    if (size_px != nullptr) {
        ...
        ESP_UI_CHECK_FALSE_RETURN(ret_size != 0, nullptr, "Font size is not found");
        *size_px = ret_size;
    }
...
bool ESP_UI_CoreHome::calibrateCoreObjectSize(const ESP_UI_StyleSize_t &parent, ESP_UI_StyleSize_t &target) const
{
    uint16_t parent_w = 0;
    uint16_t parent_h = 0;
...
bool ESP_UI_CoreHome::calibrateCoreFont(const ESP_UI_StyleSize_t *parent, ESP_UI_StyleFont_t &target) const
{
    ...
    // Size
    ESP_UI_CHECK_VALUE_RETURN(target.size_px, ESP_UI_STYLE_FONT_SIZE_MIN, ESP_UI_STYLE_FONT_SIZE_MAX, false,
                              "Invalid size");
    // Font description
    if (font_resource == nullptr) {
        font_resource = getCoreUpdateFontBySize(target.size_px);
        ESP_UI_CHECK_NULL_RETURN(font_resource, false, "Get default font failed");
...
process_height:
    ...
    if (target.flags.enable_height_percent) {
        ...
        ESP_UI_CHECK_VALUE_RETURN(target.height, 1, parent->height, false, "Invalid height");
    }
...
bool ESP_UI_CoreHome::calibrateCoreIconImage(const ESP_UI_StyleImage_t &target) const
{
    ESP_UI_CHECK_NULL_RETURN(target.resource, false, "Invalid resource");

    return true;
}

bool ESP_UI_CoreHome::processMainScreenLoad(void)
{
    ESP_UI_CHECK_FALSE_RETURN(checkCoreInitialized(), false, "Not initialized");
    ESP_UI_CHECK_FALSE_RETURN(lv_obj_is_valid(_main_screen), false, "Invalid main screen");
...
bool ESP_UI_CoreHome::beginCore(void)
{
    lv_obj_t *main_screen = nullptr;
    lv_obj_t *system_screen = nullptr;
    lv_disp_t *display = _core.getDisplayDevice();
    ESP_UI_LvObj_t main_screen_obj = nullptr;
    ESP_UI_LvObj_t system_screen_obj = nullptr;

    ESP_UI_LOGD("Begin(0x%p)", this);
    ESP_UI_CHECK_FALSE_RETURN(!checkCoreInitialized(), false, "Already initialized");
    ESP_UI_CHECK_NULL_RETURN(display, false, "Invalid display device");

    /* Create objects */
    // Main screen
    main_screen = lv_disp_get_scr_act(display);
    ESP_UI_CHECK_NULL_RETURN(main_screen, false, "Invalid lvgl current screen");
    main_screen_obj = ESP_UI_LV_OBJ(obj, main_screen);
    ESP_UI_CHECK_NULL_RETURN(main_screen_obj, false, "Create main screen failed");
    // System screen
    system_screen = lv_disp_get_layer_top(display);
    ESP_UI_CHECK_NULL_RETURN(system_screen, false, "Invalid lvgl top screen");
    system_screen_obj = ESP_UI_LV_OBJ(obj, system_screen);
    ESP_UI_CHECK_NULL_RETURN(system_screen_obj, false, "Create system screen failed");
    ...
    // Update object style
    ESP_UI_CHECK_FALSE_GOTO(updateByNewData(), err, "Update object style failed");
    ESP_UI_CHECK_FALSE_GOTO(hideContainerBorder(), err, "Hide container border failed");

    return true;

err:
    ESP_UI_CHECK_FALSE_RETURN(delCore(), false, "Delete core home failed");

    return false;
}

bool ESP_UI_CoreHome::delCore(void)
{
...
bool ESP_UI_CoreHome::updateByNewData(void)
{
    const ESP_UI_StyleSize_t &screen_size = _core.getCoreData().screen_size;

    ESP_UI_LOGD("Update core home by new data");
...
bool ESP_UI_CoreHome::calibrateCoreData(ESP_UI_CoreHomeData_t &data)
{
    ...
    for (int i = 0; i < data.text.default_fonts_num; i++) {
        ESP_UI_CHECK_VALUE_RETURN(data.text.default_fonts[i].size_px, ESP_UI_STYLE_FONT_SIZE_MIN,
                                  ESP_UI_STYLE_FONT_SIZE_MAX, false, "Invalid default font(%d) size", i);
        ESP_UI_CHECK_NULL_RETURN(data.text.default_fonts[i].font_resource, false, "Invalid default font(%d) dsc", i);
    ...
    for (int i = ESP_UI_STYLE_FONT_SIZE_MIN; i <= ESP_UI_STYLE_FONT_SIZE_MAX; i += 2) {
        if (_update_size_font_map.find(i) == _update_size_font_map.end()) {
            ESP_UI_LOGW("Default font size(%d) is not found, try to use internal font instead", i);
            if (!esp_ui_core_utils_get_internal_font_by_size(i, &font_resource)) {
                continue;
            }
...
const lv_font_t *ESP_UI_CoreHome::getCoreUpdateFontBySize(uint8_t size_px) const
{
    ESP_UI_CHECK_VALUE_RETURN(size_px, ESP_UI_STYLE_FONT_SIZE_MIN, ESP_UI_STYLE_FONT_SIZE_MAX, nullptr, "Invalid size");

    auto it = _update_size_font_map.find(size_px);
    ESP_UI_CHECK_FALSE_RETURN(it != _update_size_font_map.end(), nullptr, "Font size(%d) is not found", size_px);

    return it->second;
}

const lv_font_t *ESP_UI_CoreHome::getCoreUpdateFontByHeight(uint8_t height, uint8_t *size_px) const
{
    uint8_t ret_size = 0;
