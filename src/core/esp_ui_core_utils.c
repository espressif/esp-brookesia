...
bool esp_ui_core_utils_get_internal_font_by_size(uint8_t size_px, const lv_font_t **font)
{
    ...
    size_px = (size_px << 2) >> 2;
    size_px = MIN(MAX(size_px, ESP_UI_STYLE_FONT_SIZE_MIN), ESP_UI_STYLE_FONT_SIZE_MAX);
    if (temp_size != size_px) {
        ESP_UI_LOGW("Font size(%d) not support, use the nearest size(%d)", temp_size, size_px);
    }
    ...
    default:
        ret = false;
        ESP_UI_LOGE("No internal font size(%d) found, use default instead", temp_size);
        break;
    }
