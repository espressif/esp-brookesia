...
.. note::
   ...
   - **描述**：函数的功能说明。
   - **返回值**：函数返回值的类型与描述；仅当函数会返回数据时显示。原始 schema JSON 中始终包含 ``return_value`` 字段；其值为 ``null`` 表示函数无返回值。
...
.. note::
   ...
   - ``返回类型`` 必须与函数契约中的 **返回值** 类型一致。
   - 若函数无 **返回值** （原始 JSON 中为 ``return_value: null``），则 ``<返回类型>`` 模板参数可以省略。
...
.. note::
   ...
   - ``参数列表`` 的类型、数量和顺序必须与函数定义中的参数列表一致，若函数无参数，则省略。
   - ``on_function_handler`` 为可选参数，若省略则不处理函数结果与返回值。
   - 读取 ``result.get_data<返回类型>()`` 时，``返回类型`` 必须与函数契约中的 **返回值** 类型一致；无 **返回值** 的函数没有可解析的数据。
...
.. note::
   ...
   - **描述**：事件的功能说明。
   - **条目列表**：包含事件 payload 条目的名称、类型、描述和默认值。
      - **类型**：总共支持 ``String``、 ``Number``、 ``Boolean``、 ``Object``、 ``Array``、 ``RawBuffer`` 六种，它们与事件回调接收的条目类型关系如下：
         - ``String``： ``std::string``
         - ``Number``： ``double/int/<任意算术类型>``
         - ``Boolean``： ``bool``
         - ``Object``： ``boost::json::object``（通常可以利用 ``struct`` 序列化生成）
