...
如果希望一步生成裁剪后的字体文件，或使用 `--drop-complex-noncommon` 按字形复杂度删除笔画多的非常用字，则需要安装 fonttools：
...
### 字符复杂度过滤参数（可选，需 fonttools）

| 参数 | 说明 |
|---|---|
| `--drop-complex-noncommon` | 删除“笔画多的非常用字”以减小字体体积，需配合 `--font-path` |
| `--common-level` | 视为常用字并始终保留的 GB 级别，默认 `l1`；可选 `none` / `l1` / `l2` / `gbk` |
| `--common-file` | 额外始终保留的字符文件（如 UI 文案），可重复使用 |
| `--complex-max-points` | 字形点数阈值，非常用汉字点数超过该值即删除，默认 `100`，值越小删得越多 |
...
对于 `l2` 或 `gbk` 这类较大的字符集，二级字里包含大量很少出现、但笔画很多的生僻字（例如 `鬣`、`纛`、`爨`、`馕`、`麟`）。这些字几乎不会出现在 UI 中，却因为字形轮廓复杂而占用大量 `glyf` 空间。

`--drop-complex-noncommon` 可以在保留常用字的前提下，删除这些“笔画多的非常用字”，进一步减小字体体积。

判定逻辑（三个条件同时满足才会删除）：

- 该字符是 CJK 汉字；
- 不在常用集内（由 `--common-level` 指定，默认 GB2312 一级字）；
- 其字形轮廓的点数超过 `--complex-max-points`。

常用字、英文、数字、符号都不会被该步骤删除。

> 说明：脚本用“字形轮廓点数”作为“笔画多少”的近似度量。点数越多，通常笔画越多，在 `glyf` 表里占用的字节也越多。该度量来自 `--font-path` 指定的字体，因此此功能需要安装 `fonttools`，并且必须提供 `--font-path`。

示例（在 `l2` 基础上删除笔画多的非常用字，并直接生成子集字体）：
...
python3 charset_filter.py \
  ...
  --common-level l1 \
  --complex-max-points 100 \
  --font-path ./build/SmileySans-Oblique.ttf \
  --font-number 0 \
  --subset-output SmileySans-Oblique.subset.ttf
```
...
```text
  kept chars           : 6065
  removed chars        : 15098
  complex removed      : 959 (non-common Han, points > 100)
```

以 SmileySans 为例的实际效果：

```text
字符数：7024 → 6065（删除 959 个非常用且笔画多的汉字）
子集字体：约 1.88 MiB → 约 1.52 MiB
```

阈值调节：

- `--complex-max-points` 调小（如 `80`、`70`）会删除更多字，体积更小；
- 调大（如 `120`）则更保守，只删除最复杂的少数生僻字。

如果担心误删某些必现的 UI 文案字符，可用 `--common-file` 把这些字符所在的文件传入，它们会和常用字一样被保护：
...
python3 charset_filter.py \
  ...
  --gb-level l2 \
  --drop-complex-noncommon \
  --common-file brookesia_strings.txt \
  --font-path ./build/SmileySans-Oblique.ttf
```

> 注意：字形点数是按具体字体度量的，不同字体（如 Noto、LXGW、SmileySans）同一个字的点数略有差异，但复杂度排序基本一致。跨多个字体共用同一字符集时，可任选其中一个字体作为度量基准。
...
python3 charset_filter.py \
  ...
  --font-path /usr/share/fonts/truetype/wqy/wqy-microhei.ttc \
  --font-number 0 \
  --subset-output wqy-microhei-brookesia-dynamic-l2.ttf
```
