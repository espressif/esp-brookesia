...
CUSTOM_DEVICE_IMPLEMENT(audio_codec_ready, audio_init, esp_mosaico_audio_ready_deinit);
...
static int audio_init(void *config, int cfg_size, void **device_handle)
{
    const auto *source = get_config<dev_custom_audio_codec_ready_config_t>(config, cfg_size);
    if (source == nullptr) {
        return esp_mosaico_audio_ready_init(nullptr, device_handle);
    }
    const esp_mosaico_audio_ready_config_t parameters = {
        .settle_time_ms = source->settle_time_ms,
    };
    return esp_mosaico_audio_ready_init(&parameters, device_handle);
}
