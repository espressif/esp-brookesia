...
static esp_err_t co5300_spi_enter_sleep(void *driver_data);
static esp_err_t co5300_spi_exit_sleep(void *driver_data);
static esp_err_t co5300_spi_enter_deep_standby(void *driver_data);
static esp_err_t co5300_spi_exit_deep_standby(void *driver_data);

#define CO5300_CMD_DSTBON               0x4F
#define CO5300_DSTBON_ENTER             0x01
#define CO5300_SLEEP_CMD_GUARD_MS       5
#define CO5300_SLEEP_MODE_SETTLE_MS     120
#define CO5300_DEEP_STANDBY_RESX_MS     5
...
esp_err_t esp_lcd_new_panel_co5300_spi(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    co5300->panel_ctx.enter_sleep = co5300_spi_enter_sleep;
    co5300->panel_ctx.exit_sleep = co5300_spi_exit_sleep;
    co5300->panel_ctx.enter_deep_standby = co5300_spi_enter_deep_standby;
    co5300->panel_ctx.exit_deep_standby = co5300_spi_exit_deep_standby;
...
{
    co5300_panel_t *co5300 = (co5300_panel_t *)driver_data;
    ESP_RETURN_ON_FALSE(co5300 && co5300->io, ESP_ERR_INVALID_STATE, TAG, "panel not ready");
    ESP_RETURN_ON_ERROR(tx_param(co5300, co5300->io, LCD_CMD_SLPIN, NULL, 0), TAG, "send SLPIN failed");
    vTaskDelay(pdMS_TO_TICKS(CO5300_SLEEP_CMD_GUARD_MS));
    vTaskDelay(pdMS_TO_TICKS(CO5300_SLEEP_MODE_SETTLE_MS));
    return ESP_OK;
}
...
static esp_err_t co5300_spi_exit_sleep(void *driver_data)
{
    co5300_panel_t *co5300 = (co5300_panel_t *)driver_data;
    ESP_RETURN_ON_FALSE(co5300 && co5300->io, ESP_ERR_INVALID_STATE, TAG, "panel not ready");
    ESP_RETURN_ON_ERROR(tx_param(co5300, co5300->io, LCD_CMD_SLPOUT, NULL, 0), TAG, "send SLPOUT failed");
    vTaskDelay(pdMS_TO_TICKS(CO5300_SLEEP_CMD_GUARD_MS));
    vTaskDelay(pdMS_TO_TICKS(CO5300_SLEEP_MODE_SETTLE_MS));
    return ESP_OK;
}
...
static esp_err_t co5300_spi_enter_deep_standby(void *driver_data)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(co5300, co5300->io, CO5300_CMD_DSTBON, &dstb, 1), TAG, "send DSTBON failed");
    return ESP_OK;
}
...
static esp_err_t co5300_spi_exit_deep_standby(void *driver_data)
{
    co5300_panel_t *co5300 = (co5300_panel_t *)driver_data;
    ESP_RETURN_ON_FALSE(co5300, ESP_ERR_INVALID_ARG, TAG, "invalid panel data");
    ESP_RETURN_ON_FALSE(co5300->reset_gpio_num >= 0, ESP_ERR_NOT_SUPPORTED, TAG, "RESX GPIO required to leave deep standby");

    /* Datasheet: exit Deep Standby with RESX low pulse > 3 ms. */
    gpio_set_level(co5300->reset_gpio_num, co5300->flags.reset_level);
    vTaskDelay(pdMS_TO_TICKS(CO5300_DEEP_STANDBY_RESX_MS));
    gpio_set_level(co5300->reset_gpio_num, !co5300->flags.reset_level);
    vTaskDelay(pdMS_TO_TICKS(CO5300_SLEEP_MODE_SETTLE_MS));
    return ESP_OK;
}
