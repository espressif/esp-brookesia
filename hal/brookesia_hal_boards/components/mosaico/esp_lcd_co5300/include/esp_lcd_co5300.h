...
/**
 * @brief Enter CO5300 Sleep In mode (SLPIN, 0x10)
 *
 * Stops the DC/DC converter, internal oscillator and panel scanning. Prefer
 * sending Display Off (0x28) first for a clean blank transition.
 *
 * @note Datasheet requires ~5 ms before the next command and up to ~120 ms for
 *       the booster to fully turn off.
 */
esp_err_t esp_lcd_panel_co5300_enter_sleep(esp_lcd_panel_handle_t panel);

/**
 * @brief Exit CO5300 Sleep In mode (SLPOUT, 0x11)
 *
 * @note Datasheet requires ~5 ms before the next command and up to ~120 ms for
 *       the booster to turn on. Call Display On (0x29) afterwards if needed.
 */
esp_err_t esp_lcd_panel_co5300_exit_sleep(esp_lcd_panel_handle_t panel);

/**
 * @brief Enter CO5300 Deep Standby (DSTBON, 0x4F with DSTB=1)
 *
 * Lowest IC power state short of cutting rails. SPI/QSPI commands stop working
 * until the panel is woken with a RESX low pulse (>= 3 ms) or a full reset.
 */
esp_err_t esp_lcd_panel_co5300_enter_deep_standby(esp_lcd_panel_handle_t panel);

/**
 * @brief Exit Deep Standby by pulsing RESX low for >= 3 ms
 *
 * Does not re-run vendor init. Callers must `esp_lcd_panel_reset()` /
 * `esp_lcd_panel_init()` (and Display On) afterwards.
 */
