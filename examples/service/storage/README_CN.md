...
本示例演示了如何使用 ESP-Brookesia 的 Storage 服务进行数据存储和读取操作。示例展示了基本操作、类型安全操作、命名空间管理和错误处理等功能。

## 📑 目录

- [Storage 服务示例](#storage-服务示例)
  - [📑 目录](#-目录)
  - [✨ 功能特性](#-功能特性)
  - [🚩 快速入门](#-快速入门)
    - [硬件要求](#硬件要求)
  ...
  - [📖 示例说明](#-示例说明)
    ...
    - [错误处理演示](#错误处理演示)
  - [🔍 故障排除](#-故障排除)
    - [Storage 后端初始化失败](#storage-后端初始化失败)
    - [数据读取失败](#数据读取失败)
    - [值验证失败](#值验证失败)
  - [💬 技术支持与反馈](#-技术支持与反馈)
...
- 💾 **基本操作**：支持键值对的设置（`KVSet`）、获取（`KVGet`）、列表（`KVList`）和删除（`KVErase`）
...
搭载 `ESP32-S3`、`ESP32-P4` 或 `ESP32-C5` 芯片且 `Flash >= 4MB` 的开发板
...
1. **基本操作演示**：展示如何使用 KVSet、KVGet、KVList、KVErase 函数进行基本的键值对操作
...
演示了 Storage 服务的通用 API,包含四个基本操作：

- **KVSet（设置）**：批量设置多个键值对（字符串、整数、布尔值）
- **KVList（列表）**：列出命名空间中所有的键值对及其类型
- **KVGet（获取）**：获取指定的键值对，并验证值与设置的值一致
- **KVErase（删除）**：删除指定的键值对，并验证删除结果
...
> [!NOTE]
> 相较于使用通用的函数调用 API 进行数据存储和读取，建议使用类型安全的 API 进行数据存储和读取，因为它们更容易使用且更不容易出错。
