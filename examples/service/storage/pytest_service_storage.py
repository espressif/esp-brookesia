...
3. Build the example:
   ...
   **Build with specific target:**
   ```bash
   python .gitlab/tools/build_apps.py examples/service/storage -t esp32s3
   ```

   **Build for all targets:**
   ```bash
   python .gitlab/tools/build_apps.py examples/service/storage -t all
   ```
...
2. Run pytest with appropriate target and environment:
   ...
   **ESP32-S3 PSRAM examples:**
   ...
   pytest examples/service/storage --target esp32s3 --env "generic,octal-psram"
'''
