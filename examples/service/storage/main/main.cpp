...
extern "C" void app_main(void)
{
    ...
    BROOKESIA_CHECK_FALSE_EXIT(StorageHelper::is_available(), "Storage service is not available");
    BROOKESIA_LOGI("Storage service is available");

    // Bind Storage service to start it and its dependencies (RAII - service stays alive while binding exists)
    auto binding = service_manager.bind(StorageHelper::get_name().data());
    BROOKESIA_CHECK_FALSE_EXIT(binding.is_valid(), "Failed to bind Storage service");
    BROOKESIA_LOGI("Storage service bound successfully");
    ...
    for (const auto &nspace : nspaces) {
        auto erase_result = StorageHelper::erase_keys(nspace);
    ...
    BROOKESIA_LOGI("\n\n=== Storage Service Example Completed ===\n");
}

/**
 * @brief Demonstrate basic Storage operations: KVList, KVSet, KVGet, KVErase
 */
static bool demo_basic_operations()
{
    ...
    StorageHelper::KeyValueMap kv_pairs = {
        {"key_str", std::string("Espressif")},
        {"key_int", int32_t(100)},
        {"key_bool", bool(true)},
    };
    auto set_result = StorageHelper::call_function_sync(
                          StorageHelper::FunctionId::KVSet,
                          nspace,
    ...
    BROOKESIA_LOGI("\n\n--- Step 2: List entries ---\n");
    auto list_result = StorageHelper::call_function_sync<boost::json::array>(
                           StorageHelper::FunctionId::KVList, nspace
                       );
    ...
    auto get_result = StorageHelper::call_function_sync<boost::json::object>(
                          StorageHelper::FunctionId::KVGet,
                          nspace,
    ...
    auto erase_result = StorageHelper::call_function_sync(
                            StorageHelper::FunctionId::KVErase,
                            nspace,
    ...
    auto verify_result = StorageHelper::call_function_sync<boost::json::array>(
                             StorageHelper::FunctionId::KVList, nspace
                         );
    BROOKESIA_CHECK_FALSE_RETURN(verify_result, false, "Failed to verify after erase: %1%", verify_result.error());
    std::vector<StorageHelper::EntryInfo> verify_entries;
    BROOKESIA_CHECK_FALSE_RETURN(
...
// Complex struct for Storage operations
enum class Status {
    Idle = 0,
    Running = 1,
    Stopped = 2,
...
static bool demo_type_safe_operations()
{
    ...
    BROOKESIA_LOGI("\n\n--- Step 1: Direct storage types ---\n");
    {
        // bool
        bool bool_val = true;
        auto save_result = StorageHelper::save_key_value(nspace, "enable_feature", bool_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save bool: %1%", save_result.error());
        auto get_bool_result = StorageHelper::get_key_value<bool>(nspace, "enable_feature");
        ...
        save_result = StorageHelper::save_key_value(nspace, "int", int32_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save int32_t: %1%", save_result.error());
        auto get_int32_result = StorageHelper::get_key_value<int32_t>(nspace, "int");
        ...
        save_result = StorageHelper::save_key_value(nspace, "brightness", uint8_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save uint8_t: %1%", save_result.error());
        auto get_uint8_result = StorageHelper::get_key_value<uint8_t>(nspace, "brightness");
        ...
        save_result = StorageHelper::save_key_value(nspace, "volume", int16_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save int16_t: %1%", save_result.error());
        auto get_int16_result = StorageHelper::get_key_value<int16_t>(nspace, "volume");
    ...
    {
        // std::string
        std::string str_val = "Hello, Storage!";
        auto save_result = StorageHelper::save_key_value(nspace, "greeting", str_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save string: %1%", save_result.error());
        auto get_str_result = StorageHelper::get_key_value<std::string>(nspace, "greeting");
        ...
        save_result = StorageHelper::save_key_value(nspace, "large_number", int64_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save int64_t: %1%", save_result.error());
        auto get_int64_result = StorageHelper::get_key_value<int64_t>(nspace, "large_number");
        ...
        save_result = StorageHelper::save_key_value(nspace, "large_unsigned", uint64_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save uint64_t: %1%", save_result.error());
        auto get_uint64_result = StorageHelper::get_key_value<uint64_t>(nspace, "large_unsigned");
        ...
        save_result = StorageHelper::save_key_value(nspace, "pi_value", double_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save double: %1%", save_result.error());
        auto get_double_result = StorageHelper::get_key_value<double>(nspace, "pi_value");
        ...
        save_result = StorageHelper::save_key_value(nspace, "e_value", float_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save float: %1%", save_result.error());
        auto get_float_result = StorageHelper::get_key_value<float>(nspace, "e_value");
        ...
        save_result = StorageHelper::save_key_value(nspace, "number_list", vec_val);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save vector: %1%", save_result.error());
        auto get_vec_result = StorageHelper::get_key_value<std::vector<int>>(nspace, "number_list");
        ...
        save_result = StorageHelper::save_key_value(nspace, "complex_struct", complex_struct);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save complex struct: %1%", save_result.error());
        auto get_struct_result = StorageHelper::get_key_value<ComplexStruct>(nspace, "complex_struct");
...
static bool demo_namespace_management()
{
    ...
    {
        auto result1 = StorageHelper::save_key_value(storage_nspace, "data_key", std::string("storage_data"));
        BROOKESIA_CHECK_FALSE_RETURN(result1, false, "Failed to save to storage namespace: %1%", result1.error());

        auto result2 = StorageHelper::save_key_value(config_nspace, "data_key", std::string("config_data"));
        BROOKESIA_CHECK_FALSE_RETURN(result2, false, "Failed to save to config namespace: %1%", result2.error());

        auto result3 = StorageHelper::save_key_value(stats_nspace, "data_key", std::string("stats_data"));
    ...
    {
        auto result1 = StorageHelper::get_key_value<std::string>(storage_nspace, "data_key");
        ...
        auto result2 = StorageHelper::get_key_value<std::string>(config_nspace, "data_key");
        ...
        auto result3 = StorageHelper::get_key_value<std::string>(stats_nspace, "data_key");
...
static bool demo_error_handling()
{
    ...
    {
        auto result = StorageHelper::get_key_value<std::string>(nspace, "non_existent_key", DEMO_TIMEOUT_MS);
    ...
    BROOKESIA_LOGI("\n\n--- Step 2: Type mismatch ---\n");
    {
        ...
        auto save_result = StorageHelper::save_key_value(nspace, "type_test", std::string("test_string"), DEMO_TIMEOUT_MS);
        BROOKESIA_CHECK_FALSE_RETURN(save_result, false, "Failed to save string: %1%", save_result.error());
        // Try to get as int (should fail or return error)
        auto get_result = StorageHelper::get_key_value<int32_t>(nspace, "type_test", DEMO_TIMEOUT_MS);
