...
3. Build the example:
   ...
   **Build with specific target:**
   ```bash
   python .gitlab/tools/build_apps.py examples/service/nvs -t esp32s3
   ```

   **Build for all targets:**
   ```bash
   python .gitlab/tools/build_apps.py examples/service/nvs -t all
   ```
...
2. Run pytest with appropriate target and environment:
   ...
   **ESP32-S3 examples:**
   ```bash
   # Generic environment
   pytest examples/service/nvs --target esp32s3 --env generic
'''
...
SUCCESS_RESPONSE = b'=== NVS Service Example Completed ==='
TIMEOUT_S = 30
