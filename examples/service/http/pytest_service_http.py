...
3. Build the example:
   ...
   **Build with specific target:**
   ```bash
   python .gitlab/tools/build_apps.py examples/service/http -t esp32s3
   ```

   **Build for all targets:**
   ```bash
   python .gitlab/tools/build_apps.py examples/service/http -t all
   ```
...
2. Run pytest with appropriate target and environment:
   ...
   **ESP32-S3 PSRAM examples:**
   ...
   pytest examples/service/http --target esp32s3 --env "generic,octal-psram"
'''
...
SUCCESS_RESPONSE = b'=== Http Service Example Completed ==='
TIMEOUT_S = 10 * 60
