...
设备成功启动后，您将看到控制台提示符 `esp32s3>` 或 `esp32p4>`，表示可以开始输入命令。
...
等待看到 `WiFi Connect finished` 日志表示连接成功。

> [!TIP]
> 详细的 WiFi 命令说明请参考 [WiFi 服务文档](./cmd_wifi_cn.md)。
...
此时 Agent 会提示输入设备激活码，请在 PC 端将激活码输入到 [XiaoZhi 控制台](https://xiaozhi.me/console/agents)，设备会在一段时间后自动获取认证。
...
> [!TIP]
> 详细的 Agent 命令说明请参考 [Agent 服务文档](./cmd_agent_cn.md)。
...
> [!TIP]
> 详细的音频命令说明请参考 [音频服务文档](./cmd_audio_cn.md)。
