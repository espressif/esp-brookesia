...
public:
    PluginA() : value_(100)
    {
        BROOKESIA_LOGI("PluginA() constructed");
    }

    explicit PluginA(int val) : value_(val)
    {
        BROOKESIA_LOGI("PluginA(%1%) constructed", val);
    }

    ~PluginA() override
    {
        BROOKESIA_LOGI("PluginA destroyed");
    }
...
public:
    PluginB() : value_(200)
    {
        BROOKESIA_LOGI("PluginB() constructed");
    }

    explicit PluginB(int val) : value_(val)
    {
        BROOKESIA_LOGI("PluginB(%1%) constructed", val);
    }

    ~PluginB() override
    {
        BROOKESIA_LOGI("PluginB destroyed");
    }
...
public:
    ...
    {
        BROOKESIA_LOGI("PluginC(%1%, %2%) constructed", name, val);
    }

    ~PluginC() override
    {
        BROOKESIA_LOGI("PluginC destroyed");
    }
...
TEST_CASE("Test multiple plugins", "[utils][plugin_registry][multiple]")
{
    ...
    // Verify plugin count
    TEST_ASSERT_EQUAL(2, PluginRegistry<IPlugin>::get_plugin_count());
...
TEST_CASE("Test remove operations", "[utils][plugin_registry][remove]")
{
    ...
    TEST_ASSERT_EQUAL(1, PluginRegistry<IPlugin>::get_plugin_count());
...
TEST_CASE("Test thread safety", "[utils][plugin_registry][thread_safety]")
{
        ...
        futures.push_back(std::async(std::launch::async, [ &, t]() {
            for (int i = 0; i < operations_per_thread; i++) {
                try {
                    ...
                    // Test 3: Concurrent query plugin count
                    if (PluginRegistry<IPlugin>::get_plugin_count() == 3) {
                        success_count++;
                    } else {
                        error_count++;
                    }
    ...
    // Verify plugin state is not corrupted
    TEST_ASSERT_EQUAL(3, PluginRegistry<IPlugin>::get_plugin_count());
    auto plugin_a = PluginRegistry<IPlugin>::get_instance("plugin_a");
    TEST_ASSERT_NOT_NULL(plugin_a);
    TEST_ASSERT_EQUAL(100, plugin_a->get_value());
...
TEST_CASE("Test edge cases", "[utils][plugin_registry][edge_cases]")
{
    ...
    // Clean up (already empty, but maintain consistency)
    PluginRegistry<IPlugin>::remove_all_plugins();
}
...
TEST_CASE("Test complex scenario", "[utils][plugin_registry][complex]")
{
    ...
    TEST_ASSERT_EQUAL(2, all_instances.size());
