...
static const char *TAG = "rainmaker_app_backend_util";

#define RM_APP_NVS_NS "rm_app"

static const rainmaker_app_backend_util_tz_db_pair_t s_tz_db[] = {
    {"Africa/Abidjan", "GMT0"},
    {"Africa/Accra", "GMT0"},
    ...
    {"America/Gnome", "AKST9AKDT,M3.2.0,M11.1.0"},
...
const char *rainmaker_app_backend_util_get_tz_posix_str(const char *tz_name)
{
    ...
    for (size_t i = 0; i < sizeof(s_tz_db) / sizeof(s_tz_db[0]); i++) {
        if (strcmp(s_tz_db[i].name, tz_name) == 0) {
            return s_tz_db[i].posix_str;
        }
    }
    return NULL;
}
...
void rainmaker_app_backend_util_get_tz_db(const rainmaker_app_backend_util_tz_db_pair_t **out_db, size_t *out_count)
{
    ...
    if (out_count) {
        *out_count = sizeof(s_tz_db) / sizeof(s_tz_db[0]);
    }
}
...
esp_err_t rainmaker_app_backend_util_nvs_read_str(const char *key, char **out_str)
{
    ...
    *out_str = NULL;
    ...
    nvs_handle_t h;
    esp_err_t err = nvs_open(RM_APP_NVS_NS, NVS_READONLY, &h);
    if (err != ESP_OK) {
        return err;
    }
...
esp_err_t rainmaker_app_backend_util_nvs_write_str(const char *key, const char *val)
{
    ...
    nvs_handle_t h;
    esp_err_t err = nvs_open(RM_APP_NVS_NS, NVS_READWRITE, &h);
    if (err != ESP_OK) {
        goto cleanup;
    }
