...
static const char *TAG = "rainmaker_standard_switch";
...
esp_err_t rainmaker_standard_switch_set_power(const char *node_id, bool power_on, const char **out_err_reason)
{
    ...
    char payload[128] = {0};
    snprintf(payload, sizeof(payload), "{\"Switch\":{\"Power\":%s}}", power_on ? "true" : "false");
...
esp_err_t rainmaker_standard_switch_set_name(const char *node_id, const char *name, const char **out_err_reason)
{
    ...
    char payload[128] = {0};
    snprintf(payload, sizeof(payload), "{\"Switch\":{\"Name\":\"%s\"}}", name);
    return rainmaker_api_set_device_parameters(node_id, payload, out_err_reason);
}

esp_err_t rainmaker_standard_switch_parse_parameters(cJSON *device, rm_device_item_t *device_item)
{
    ...
    device_item->type = RAINMAKER_APP_DEVICE_TYPE_SWITCH;
    return ESP_OK;
}
