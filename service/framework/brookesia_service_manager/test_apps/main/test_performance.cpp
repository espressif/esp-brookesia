...
BROOKESIA_TEST_CASE(test_performance_call_function, "Test Performance: call function", "[brookesia][service][call_function_sync]")
{
...
BROOKESIA_TEST_CASE(test_performance_concurrent_call_function, "Test Performance: concurrent call function", "[brookesia][service][call_function_sync][concurrent]")
{
...
BROOKESIA_TEST_CASE(test_performance_with_scheduler_concurrent_call_function, "Test Performance with scheduler: concurrent call function", "[brookesia][service][call_function_sync][concurrent][with_scheduler]")
{
    ConcurrentCallConfig concurrent_config{};
    bool result = false;
    {
...
)
{
    ...
    while (remaining_futures > 0) {
        ...
        if (!any_ready) {
            esp_brookesia::lib_utils::test_adapter::delay_ms(1);
        }
    }
