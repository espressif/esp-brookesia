...
FunctionResult FunctionRegistry::call(const std::string &func_name, FunctionParameterMap parameters)
{
    ...
    // Call the function
    FunctionResult result = func_handler(std::move(validated_parameters));
    BROOKESIA_CHECK_FALSE_RETURN(
        validate_return_value(func_schema, result, error_message), error_result, "%1%", error_message
    );
    ...
    for (const auto &param : func_schema.parameters) {
        ...
        if (it == parameters.end()) {
            ...
            if (!param.is_compatible_value(it->second)) {
                ...
                const auto actual_type = get_function_value_type(it->second);
...
bool FunctionRegistry::validate_return_value(
    const FunctionSchema &func_schema, const FunctionResult &result, std::string &error_msg
)
{
    BROOKESIA_LOG_TRACE_GUARD_WITH_THIS();
    ...
    if (!func_schema.return_value.has_value()) {
        if (result.has_data()) {
            error_msg = "Function `" + func_schema.name + "` returned data but schema declares no return value";
            BROOKESIA_LOGE("%1%", error_msg);
            return false;
        }
        return true;
    ...
    if (!result.has_data()) {
        error_msg = "Function `" + func_schema.name + "` returned no data but schema declares return type `" +
                    BROOKESIA_DESCRIBE_TO_STR(func_schema.return_value->type) + "`";
        BROOKESIA_LOGE("%1%", error_msg);
        return false;
    }

    const auto actual_type = get_function_value_type(*result.data);
    if (actual_type != func_schema.return_value->type) {
        error_msg = "Function `" + func_schema.name + "` returned invalid data type: expected `" +
                    BROOKESIA_DESCRIBE_TO_STR(func_schema.return_value->type) + "`, but got `" +
                    BROOKESIA_DESCRIBE_TO_STR(actual_type) + "`";
        BROOKESIA_LOGE("%1%", error_msg);
        return false;
    }
