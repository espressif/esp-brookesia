...
/* Load a ROM from a caller-owned memory buffer */
int nes_loadmem(uint8 *data, size_t size, const char *filename)
{
    rom_free();

    rom_t *cart = rom_loadmem(data, size);
    if (cart == NULL)
        return -1;
    ...
    if (filename)
    {
        ...
        if (cart->system == SYS_UNKNOWN &&
            (strstr(filename, "(E)") || strstr(filename, "(Europe)") ||
             strstr(filename, "(A)") || strstr(filename, "(Australia)")))
            cart->system = SYS_NES_PAL;
    }

    return nes_insertcart(cart);
}
