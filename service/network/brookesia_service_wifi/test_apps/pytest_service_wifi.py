...
3. (Optional) Setup WiFi credentials:
   ```bash
   export CI_TEST_WIFI_2_4G_AP1_SSID="your_wifi_ssid"
   export CI_TEST_WIFI_2_4G_AP1_PSW="your_wifi_password"
   export CI_TEST_WIFI_2_4G_AP2_SSID="your_wifi_ssid_2"
   export CI_TEST_WIFI_2_4G_AP2_PSW="your_wifi_password_2"
   ```
...
4. Build the test app:
   ...
   **Build for a specific target (replace `esp32s3` with your target chip: `esp32s3` or `esp32p4`):**
   ```bash
   python .gitlab/tools/build_apps.py service/network/brookesia_service_wifi/test_apps -t esp32s3
   ```

   **Build for all CI targets (recommended for CI):**
   ```bash
   python .gitlab/tools/build_apps.py service/network/brookesia_service_wifi/test_apps -t all
   ```
...
2. Run pytest with appropriate target and environment:
   ...
   **ESP32-P4 examples:**
   ...
   pytest service/network/brookesia_service_wifi/test_apps --target esp32p4 --env "esp32p4x_function_ev_board"
   ```
'''
import pytest
