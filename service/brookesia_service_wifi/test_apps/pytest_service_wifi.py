...
4. Build the test app:
   ...
   **Build for a specific target (replace `esp32s3` with your target chip: `esp32s3` or `esp32p4`):**
   ```bash
   python .gitlab/tools/build_apps.py service/brookesia_service_wifi/test_apps -t esp32s3
   ```

   **Build for all CI targets (recommended for CI):**
   ```bash
   python .gitlab/tools/build_apps.py service/brookesia_service_wifi/test_apps -t all
   ```
