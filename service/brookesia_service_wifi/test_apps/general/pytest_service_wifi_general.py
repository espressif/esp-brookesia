...
#   - idf-build-apps build -t esp32s3 --manifest-files=".build-rules.yml" --path='./service/brookesia_service_wifi/test_apps/general' --recursive --build-dir="@v/build_@t_@w"
#
# - Test
#   - ${IDF_PATH}/install.sh --enable-pytest
#   - ${IDF_PATH}/install.sh --enable-test-specific
#   - pytest service/brookesia_service_wifi/test_apps/general --target esp32s3 --env generic
# '''
...
def test(dut: Dut)-> None:
    ...
    response = dut.expect(ENTER_RESPONSE_LIST, return_what_before_match=True, timeout=TIMEOUT_S)
