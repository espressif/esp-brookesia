...
3. Build the test app:
   ...
   **Build for a specific board:** (replace `esp32p4` and `esp32_p4x_function_ev` with your target chip and board name)
   ```bash
   python .gitlab/tools/build_apps.py service/brookesia_service_audio/test_apps -t esp32p4 --config "sdkconfig.ci.board.esp32_p4x_function_ev=esp32_p4x_function_ev"
   ```
...
2. Run pytest with appropriate target and environment:
   ...
   **esp32p4x_function_ev_board examples:**
   ```bash
   pytest service/brookesia_service_audio/test_apps --target esp32p4 --env esp32p4x_function_ev_board
   ```
'''
