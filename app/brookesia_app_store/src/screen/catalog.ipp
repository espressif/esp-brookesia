    ...
    for (const auto &[_, ref] : instance_to_entry_) {
        if (ref.kind == VisibleItemKind::Store && ref.index < entries_.size()) {
            ...
            if (entry.update_available) {
                detail = "Installed: " + entry.installed_version + " | Latest: " + entry.latest_version;
            } else if (!entry.latest_version.empty()) {
                detail = "Latest: " + entry.latest_version;
            }
            if (!entry.updated_at.empty()) {
                if (!detail.empty()) {
                    detail += " | ";
                }
                detail += "Updated: " + entry.updated_at;
            }
            if (!entry.categories.empty()) {
                if (!detail.empty()) {
                    detail += " | ";
                }
                detail += "Categories: " + string_array_join(entry.categories);
            }
            ...
            if (!entry.app.manifest.version.empty()) {
                detail += " | Version: " + entry.app.manifest.version;
            }
            ...
            for (const auto &remote_entry : entries_) {
                if (remote_entry.package_name == entry.app.manifest.id && remote_entry.update_available) {
                    detail += " | Update available: " + remote_entry.latest_version;
                    state = tr("state.update_available");
                    break;
                }
            }
            detail += " | State: ";
            detail += BROOKESIA_DESCRIBE_TO_STR(entry.app.state);
            add_item(
                entry.item_path,
                title,
